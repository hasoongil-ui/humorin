    ...
    <>
      ...
      <header className="bg-white px-4 border-b border-gray-200 shadow-sm relative z-30 h-[68px] flex items-center">
        <div className="w-full max-w-[1200px] mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 md:gap-4 h-[32px]">
            {user ? (
              <>
                <div className="text-[13px] md:text-[14px] font-medium text-gray-700 hidden sm:flex items-center gap-1.5"><span className={`font-black ${tierInfo.color}`}>{tierInfo.icon} [{tierInfo.name}]</span><Link href={`/user/${user.nickname}`} className="font-bold text-[#3b4890] hover:underline cursor-pointer">{user.nickname}</Link><span className="text-rose-500 font-bold text-[13px]">({user.points.toLocaleString()} P)</span></div>
      ...
      <nav className="bg-[#414a66] text-gray-200 shadow-md relative z-20 min-h-[48px] md:min-h-[52px]">
        <div className="max-w-[1200px] mx-auto relative group">
          ...
          {/* 💡 [핵심 수술 구역] 우측 화살표를 [≡ 전체] 버튼 너비(80px)만큼 왼쪽으로 꺼내옵니다! */}
          {showRightArrow && (
          ...
          <div 
            ...
            className="flex items-center overflow-x-auto whitespace-nowrap hide-scrollbar relative py-0.5"
          >
            ...
            {menuGroups.map((group: any) => {
              ...
              return (
                <div 
                  key={group.name} 
                  className="group shrink-0 inline-block"
            ...
            <div className="w-28 shrink-0"></div> {/* 화살표와 겹치지 않게 여백 추가 */}
          </div>
          ...
          return createPortal(
            <div 
              ...
              style={{
                ...
                width: `${menuRect.width < 200 ? 200 : menuRect.width}px`
              }}
            ...
            >
              {currentGroup.sub?.map((subItem: any) => { 
                if (subItem.isSpecial) {
                  return (
                    <Link key={subItem.name} href={subItem.link} className="w-full text-left block px-5 py-3 text-[13px] font-bold border-t-2 border-gray-100 transition-colors bg-rose-50 text-rose-500 hover:bg-rose-100">
                      {subItem.name}
                    </Link>
                  );
                }
                ...
                return (
                  <Link key={subItem.name} href={subItem.link || ''} className={`block px-5 py-3 text-[13px] font-bold border-b border-gray-100 transition-colors last:border-0 ${isActive ? 'bg-indigo-50 text-[#3b4890]' : 'text-gray-700 hover:bg-gray-50 hover:text-[#3b4890]'}`}>
                    {subItem.name}
                  </Link>
                );
              })}
