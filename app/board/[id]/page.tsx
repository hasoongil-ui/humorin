...
export default async function PostDetailPage(props: any) {
  ...
  try {
    ...
    if (relatedPosts.length > 0) {
      ...
      const contentsResult = await Promise.all(
        selectedIds.map(id => sql`SELECT id, content FROM posts WHERE id = ${id}`)
      );

      const contentMap = new Map();
      contentsResult.forEach(res => {
        if (res.rows.length > 0) contentMap.set(res.rows[0].id, res.rows[0].content);
      });

      relatedPosts = relatedPosts.map((rp: any) => {
        const content = contentMap.get(rp.id) || '';
        let thumb = null;

        const imgMatch = content.match(/<img[^>]+src=["']([^"']+)["']/i);
        if (imgMatch) {
          thumb = { type: 'image', url: imgMatch[1] };
        } else {
          const ytMatch = content.match(/<iframe[^>]*src=["'](?:https?:)?\/\/(?:www\.)?(?:youtube\.com\/embed\/|youtu\.be\/)([^"'>?]+)/i);
          if (ytMatch && ytMatch[1]) {
            thumb = { type: 'youtube', url: `https://img.youtube.com/vi/${ytMatch[1]}/hqdefault.jpg` };
          } else {
            const posterMatch = content.match(/<video[^>]+poster=(["'])(.*?)\1/i);
            if (posterMatch && posterMatch[2]) {
              thumb = { type: 'image', url: posterMatch[2] };
            } else {
  ...
  try {
    if (keyword) {
      ...
      const countRes = await sql`SELECT COUNT(*) FROM (SELECT 1 FROM posts p INNER JOIN boards b ON p.category = b.name WHERE p.likes >= 10 AND COALESCE(p.dislikes, 0) < ${bestCutoff} AND COALESCE(p.status, 'published') = 'published' AND (${isAll}::boolean = false OR b.is_all_visible = true) LIMIT 10000) AS sub`;
      totalListCount = Number(countRes.rows[0].count);
      ...
      const countRes = await sql`SELECT COUNT(*) FROM (SELECT 1 FROM posts p INNER JOIN boards b ON p.category = b.name WHERE p.likes >= 100 AND COALESCE(p.dislikes, 0) < ${bestCutoff} AND COALESCE(p.status, 'published') = 'published' AND (${isAll}::boolean = false OR b.is_all_visible = true) LIMIT 10000) AS sub`;
      totalListCount = Number(countRes.rows[0].count);
      ...
      const countRes = await sql`SELECT COUNT(*) FROM (SELECT 1 FROM posts p INNER JOIN boards b ON p.category = b.name WHERE p.likes >= 1000 AND COALESCE(p.dislikes, 0) < ${bestCutoff} AND COALESCE(p.status, 'published') = 'published' AND (${isAll}::boolean = false OR b.is_all_visible = true) LIMIT 10000) AS sub`;
      totalListCount = Number(countRes.rows[0].count);
    ...
    if (!isAdmin) {
      try {
        ...
        if (userPoints < 10 && hasLink) {
          return {
            error: 'newbie_link', message: '스팸 방지를 위해 활동 점수 10점 미만은 링크를 포함할 수 없습니다.'
          };
        }
      } catch (e) { }
    }
  ...
  // 💡 [SEO 핀셋 패치]: 구조화된 데이터(Schema.org) 생성 (화면 덜컹거림 0%)
  const schemaData = {
    "@context": "https://schema.org",
    "@type": "DiscussionForumPosting",
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://www.humorin.kr/board/${postId}`
    },
                    ...
                    <div className="w-full relative pt-[56.25%] bg-gray-50 border-b border-gray-100 shrink-0 overflow-hidden">
                      {rp.thumb ? (
                        ...
                        <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
                          <span className="font-black text-[22px] tracking-tight text-gray-300 opacity-60">유머<span className="text-yellow-400">in</span></span>
                        </div>
                      )}
                    </div>

                    <div className="p-4 flex flex-col flex-grow">
                      <div className="text-[14px] font-bold text-gray-800 group-hover:text-[#3b4890] break-all break-words leading-snug mb-4">
                        {rpData.cleanTitle}
                      </div>
                      
                      <div className="flex justify-between items-center text-[12px] font-medium text-gray-500 mt-auto pt-2">
