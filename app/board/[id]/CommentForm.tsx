...
export default function CommentForm({ postId, parentId, author, actionType, submitAction }: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        // 💡 [안전장치] 글도 없고 사진도 없으면 여기서 차단됩니다! (required가 없어도 안전!)
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden flex flex-col mt-2">
            ...
            {/* 💡 [수술] required 속성을 삭제해서 이미지만 올려도 통과되게 만들었습니다! */}
            <textarea
                value={content}
            ...
            <div className="bg-gray-50 border-t border-gray-100 px-3 py-2 flex justify-between items-center gap-2">
                <div>
                    ...
                    <label htmlFor={uniqueId} className={`cursor-pointer px-3 py-1.5 bg-white border border-gray-300 text-gray-600 text-[12px] font-bold rounded-sm hover:bg-gray-100 shadow-sm flex items-center gap-1 ${isSubmitting ? 'opacity-50' : ''}`}>
                ...
                <div className="flex gap-2">
                    {actionType === 'reply' && (
                        <label htmlFor={`reply-${parentId}`} className="cursor-pointer px-4 py-1.5 bg-white border border-gray-300 text-gray-600 text-[12px] font-bold rounded-sm hover:bg-gray-100 shadow-sm flex items-center justify-center">
                            취소
                        </label>
                    )}
                    <button type="submit" disabled={isSubmitting} className="px-5 py-1.5 bg-[#414a66] text-white text-[13px] font-bold rounded-sm hover:bg-[#2a3042] shadow-sm disabled:bg-gray-400 flex items-center gap-1">
