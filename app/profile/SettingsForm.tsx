...
export default function SettingsForm({ 
  ...
  const handleNickBlur = async () => {
    ...
    if (cleanVal === currentNickname) { setNickError(''); setNickOk(true); return; }
  ...
  const handleEmailBlur = async () => {
    ...
    if (val === currentEmail) { setEmailError(''); setEmailOk(true); return; }
  ...
  const validatePassword = (pw: string, confirmPw: string) => {
    ...
    if (pw && pw.length < 8) { setPasswordError('❌ 비밀번호는 8자리 이상이어야 합니다.'); return; }
    if (pw && confirmPw && pw !== confirmPw) { setPasswordError('❌ 비밀번호가 서로 일치하지 않습니다.'); return; }
    setPasswordError('');
  };

  const handleSubmit = async (formData: FormData) => {
    if (nickError || emailError || passwordError) {
      alert("입력하신 정보에 오류가 있습니다.");
      return; 
    }
    ...
    <div>
      <form action={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input name="newNickname" placeholder={`현재: ${currentNickname}`} value={nickname} onChange={(e) => { setNickname(e.target.value); setNickError(''); setNickOk(false); }} onBlur={handleNickBlur} className={`w-full p-3 border rounded-sm focus:outline-none font-medium ${nickError ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#3b4890]'}`} />
        ...
        <div>
          ...
          <input name="newEmail" placeholder={currentEmail ? `현재: ${currentEmail}` : '등록된 이메일 없음'} value={isSocialUser ? (currentEmail || '') : email} disabled={isSocialUser} onChange={(e) => { if (!isSocialUser) { setEmail(e.target.value); setEmailError(''); setEmailOk(false); } }} onBlur={handleEmailBlur} className={`w-full p-3 border rounded-sm focus:outline-none font-medium ${isSocialUser ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed' : emailError ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#3b4890]'}`} />
        ...
        {isSocialUser ? (
          <div className="bg-indigo-50 p-4 border border-indigo-100 rounded-sm">
            ...
            <p className="text-xs text-gray-600 font-medium leading-relaxed">소셜 간편 가입 회원님은 비밀번호를 입력할 필요가 없습니다. 안전한 계정 보안 관리는 가입하신 <b>해당 소셜 플랫폼(네이버/카카오/구글)</b>을 이용해 주세요!</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">새 비밀번호</label>
              <input type="password" name="newPassword" value={password} onChange={(e) => { setPassword(e.target.value); validatePassword(e.target.value, passwordConfirm); }} placeholder="변경할 비밀번호 입력 (8자리 이상)" className="w-full p-3 border border-gray-300 rounded-sm focus:outline-none focus:border-[#3b4890] font-medium" />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">새 비밀번호 확인</label>
              <input type="password" value={passwordConfirm} onChange={(e) => { setPasswordConfirm(e.target.value); validatePassword(password, e.target.value); }} placeholder="비밀번호 다시 입력" className={`w-full p-3 border rounded-sm focus:outline-none font-medium ${passwordError ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#3b4890]'}`} />
              {passwordError && <p className="text-red-500 text-[12px] font-bold mt-1.5">{passwordError}</p>}
              {!passwordError && password && password === passwordConfirm && <p className="text-green-600 text-[12px] font-bold mt-1.5">✅ 비밀번호가 일치합니다.</p>}
            </div>
          </div>
        )}
        
        <button type="submit" disabled={isSubmitting || !!nickError || !!emailError || !!passwordError} className="w-full flex justify-center items-center gap-2 py-3.5 bg-[#2a3042] text-white rounded-sm font-bold text-[15px] hover:bg-[#1e2335] shadow-sm transition-colors mt-4 disabled:bg-gray-400">
